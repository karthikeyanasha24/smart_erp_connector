#!/usr/bin/env python3
"""
Analytics YTD tab — same API path the UI uses for instant toggle (like MTD).

Simulates Analytics.tsx when you click YTD:
  1. Lean GET /analytics/bundle?period=ytd&include_departments=false&include_kpis=false
  2. (optional) GET /analytics/departments + /analytics/dashboard in parallel

When step 1 is fast (~2–15s warm), the Analytics page should paint KPIs/charts
immediately on YTD toggle (same behavior as MTD on reload).

Usage:
  python test/analytics_ytd_instant.py --email you@example.com --password secret
  python test/analytics_ytd_instant.py --email you@example.com --password secret --full
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="YTD analytics UI path timing")
    ...
    if ms1 > 20_000:
        ...
        _log("  Second run should be ~2–15s (then Analytics YTD tab is instant).")
    else:
        _log("\n  ✓ Fast enough — Analytics YTD toggle should show data right away when cache is warm.")
